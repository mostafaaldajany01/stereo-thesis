...
X_FILTER_ENABLED  = True
X_FILTER_RANGE    = 1200

Y_FILTER_ENABLED  = True
Y_FILTER_RANGE    = 1000
...
def load_fs_model():
    ...
    model.cuda().eval()
    print('[FS] model loaded.')
    return model, args


def run_fs(rect_l_bgr, rect_r_bgr, model, args):
    img0 = rect_l_bgr[:, :, ::-1].copy()
    img1 = rect_r_bgr[:, :, ::-1].copy()
...
def process_pair(idx, rect_l, rect_r, cam_matrix, baseline, model, args, out_dir):
    disp = run_fs(rect_l, rect_r, model, args)

    vis = vis_disparity(disp, color_map=cv.COLORMAP_TURBO)
    cv.imwrite(os.path.join(out_dir, f'disparity_{idx}.png'), vis[:, :, ::-1])

    if REMOVE_INVISIBLE:
        yy, xx = np.meshgrid(np.arange(disp.shape[0]), np.arange(disp.shape[1]), indexing='ij')
        disp[xx - disp < 0] = np.inf

    K  = cam_matrix.astype(np.float32).copy()
    K[:2] *= SCALE
    ...
    if SOR_ENABLED and len(np.asarray(pcd.points)) > SOR_NB_NEIGHBORS:
...
def process_baseline(baseline: str, model, args):
    ...
    if not left_paths:
        print(f'  [skip] No stereo pairs in {pairs_dir}/cam0')
        return

    os.makedirs(out_dir, exist_ok=True)
    cam0, dist0, cam1, dist1, R, T = get_calibration(baseline, baseline_dir)
    bl_mm = abs(float(T[0][0]))

    size = cv.imread(left_paths[0]).shape[:2][::-1]
    R1, R2, P1, P2, _, _, _ = cv.stereoRectify(cam0, dist0, cam1, dist1, size, R, T, alpha=0)
    ...
    for idx, (lp, rp) in enumerate(zip(left_paths, right_paths), start=1):
        ...
        process_pair(idx, rect_l, rect_r, cam0, bl_mm, model, args, out_dir)

def main():
    set_seed(0)
    torch.autograd.set_grad_enabled(False)

    images_root = os.path.join(os.getcwd(), IMAGES_ROOT)
    baselines = sorted([
    ...
    model, args = load_fs_model()

    for baseline in baselines:
        print(f'\n{"=" * 60}')
        print(f'Baseline: {baseline}')
        print('=' * 60)
        process_baseline(baseline, model, args)
